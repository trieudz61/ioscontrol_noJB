...
> Đọc file này để không lặp lại thảo luận cũ.
> Cập nhật: 2026-04-03
...
| 11 | 2026-04-02 | ICToastService — standalone UIApp với BackBoard bootstrap | Daemon không có UIWindow context. Cần process riêng `posix_spawn` từ App, bootstrap UIKit qua `BKSDisplayServicesStart` + `UIApplicationInitialize` + `UIApplicationInstantiateSingleton` | `ICToastService.m` + `ICToastService/` bundle |
...
| 16 | 2026-04-03 | ICToastService watchdog: `kill(pid, 0)` verify before using PID file | PID file tồn tại ≠ process alive. Must verify with `kill(pid, 0) == 0` trước khi dùng PID. | `IOSControlDaemon.m` toastWatchdog |
| 17 | 2026-04-03 | Autocomplete: insert `func()` với cursor trong ngoặc, không thêm ngoặc khi đã có `(` | CodeMirror `hint` function check char trước cursor. Nếu đã là `(` thì strip signature `func()` → chỉ insert `func`. Tránh `sys.toast((`. | `static/script_edit.html` autocomplete |
| 18 | 2026-04-03 | Phase 12: Web IDE polish — TXT files, full signature autocomplete, Lucide icons, shortcuts drawer removal | Tối ưu UX. CodeMirror autocomplete hiện đầy đủ API signature. Lucide thay Material Icons. | `ICScriptManager.m`, `static/script_edit.html`, `static/style.css` |
