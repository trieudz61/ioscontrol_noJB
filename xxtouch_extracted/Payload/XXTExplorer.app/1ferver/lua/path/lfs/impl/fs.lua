...
_M.symlinkattributes = lfs.symlinkattributes or lfs.attributes

-- function _M.flags(P) end

local attrib = lfs.attributes

local symattrib = lfs.symlinkattributes or attrib

function _M.ctime(P)   return symattrib(P,'change') end

function _M.atime(P)   return symattrib(P,'access') end

function _M.mtime(P)   return symattrib(P,'modification') end

function _M.size(P)    return symattrib(P,'size') end

function _M.exists(P)  return symattrib(P,'mode') ~= nil and P end

function _M.isdir(P)   return symattrib(P,'mode') == 'directory' and P end

function _M.isfile(P)  return symattrib(P,'mode') == 'file' and P end

function _M.islink(P)  return symattrib(P,'mode') == 'link' and P end
...
local function do_foreach_recurse(base, match, callback, option)
  ...
  for name in dir_next, dir do if not isdots(name) then
    ...
    if _M.symlinkattributes(path,"mode") == "directory" then
...
foreach_impl = function(base, match, callback, option)
  ...
  for name in dir_next, dir do if option.skipdots == false or not isdots(name) then
    local path = base .. DIR_SEP .. name
    local attr = _M.symlinkattributes(path)
