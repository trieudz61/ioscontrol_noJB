local _M = require "alien_c"
local alien = _M
local io = require "io"

local pairs, ipairs = pairs, ipairs
local setmetatable = setmetatable
local error = error
local pcall = pcall
local type = type
local rawset = rawset
local unpack = table.unpack
local math = math

local load, callback = alien.load, alien.callback

_M.loaded = {}

local load_library, find_library = {}, {}

function find_library.linux(libname)
  local name = find_library_helper(libname, "-p")
  if name then
    local ok, lib = pcall(load, name)
    if ok then
      return lib
    end
  end
  return nil
end

function find_library.bsd(libname)
  local name = find_library_helper(libname, "-r")
  if name then
    local ok, lib = pcall(load, name)
    if ok then
      return lib
    end
  end
  return nil
end

function find_library.darwin(libname)
  local ok, lib = pcall(load, libname .. ".dylib")
  if ok then return lib end
  ok, lib = pcall(load, "/lib/"..libname .. ".dylib")
  if ok then return lib end
  ok, lib = pcall(load, "/System/Library/Frameworks/"..libname .. ".framework/" .. libname)
  if ok then return lib end
  ok, lib = pcall(load, "/Library/Frameworks/"..libname .. ".framework/" .. libname)
  if ok then return lib end
  ok, lib = pcall(load, "/System/Library/PrivateFrameworks/"..libname .. ".framework/" .. libname)
  if ok then return lib end
  ok, lib = pcall(load, "/Library/PrivateFrameworks/"..libname .. ".framework/" .. libname)
  if ok then return lib end
  return nil
end

local function load_library_helper(libname, libext)
  if libname:match("/") or libname:match("%" .. libext) then
    return load(libname)
  else
    local ok, lib = pcall(load, "lib" .. libname .. libext)
    if not ok then
      ok, lib = pcall(load, "./lib" .. libname .. libext)
      if ok then return lib end
      ok, lib = pcall(load, "/lib/lib" .. libname .. libext)
      if ok then return lib end
      ok, lib = pcall(load, "/usr/lib/lib" .. libname .. libext)
      if not ok then
        lib = find_library[alien.platform](libname)
        if lib then
          return lib
        else
          return nil, "library " .. libname .. " not found"
        end
      end
    end
    return lib
  end
end

function load_library.linux(libname)
  local lib, errmsg = load_library_helper(libname, ".so")
  if not lib then error (errmsg) end
  return lib
end

load_library.bsd = load_library.linux

function load_library.darwin(libname)
  local lib, errmsg = load_library_helper(libname, ".dylib")
  if not lib then
    lib, errmsg = load_library_helper(libname, ".so")
  end
  if not lib then error (errmsg) end
  return lib
end

function load_library.windows(libname)
  return load(libname)
end

setmetatable(_M.loaded, { __index = function (t, libname)
                                      local lib = load_library[alien.platform](libname)
                                      t[libname] = lib
                                      return lib
                                    end, __mode = "kv" })

function _M.load(libname)
  return _M.loaded[libname]
end

function _M.callback(f, ...)
  local cb = callback(f)
  cb.types(cb, ...)
  return cb
end

local array_methods = {}

local function array_next(arr, i)
  if i < arr.length then
    return i + 1, arr[i + 1]
  else
    return nil
  end
end

function array_methods:ipairs()
  return array_next, self, 0
end

function array_methods:realloc(newlen)
  self.buffer:realloc(newlen * self.size)
  self.length = newlen
end

local function array_get(arr, key)
  if type(key) == "number" then
    if key < 1 or key > arr.length then
      error("array access out of bounds")
    end
    local offset = (key - 1) * arr.size + 1
    return arr.buffer:get(offset, arr.type)
  else
    return array_methods[key]
  end
end

local function array_set(arr, key, val)
  if type(key) == "number" then
    if key < 1 or key > arr.length then
      error("array access out of bounds")
    end
    local offset = (key - 1) * arr.size + 1
    arr.buffer:set(offset, val, arr.type)
    if arr.type == "*" then
      arr.pinned[key] = val
    end
  else
    rawset(arr, key, val)
  end
end

function _M.array(t, length, init)
  local ok, size = pcall(alien.sizeof, t)
  if not ok then
    error("type " .. t .. " does not exist")
  end
  if type(length) == "table" then
    init = length
    length = #length
  end
  local arr = { type = t, length = length, size = size, pinned = {} }
  -- FIXME: When Lua 5.1 support is dropped, add a __len metamethod
  setmetatable(arr, { __index = array_get, __newindex = array_set })
  if type(init) == "userdata" then
    arr.buffer = init
  else
    arr.buffer = alien.buffer(size * length)
    if type(init) == "table" then
      for i = 1, length do
        if type(init[i]) == "string" then
          local offset = (i - 1) * size + 1
          arr.pinned[i] = alien.buffer(#init[i] + 1)
          arr.buffer:set(offset, arr.pinned[i], "*")
        end
        arr[i] = init[i]
      end
    end
  end
  return arr
end

local function struct_new(s_proto, ptr)
  local buf = alien.buffer(ptr or s_proto.size)
  local function struct_get(_, key)
    if s_proto.offsets[key] then
      if type(s_proto.types[key]) == "number" then
        return buf:topointer(s_proto.offsets[key] + 1)
      else
        return buf:get(s_proto.offsets[key] + 1, s_proto.types[key])
      end
    else
      error("field " .. key .. " does not exist")
    end
  end
  local function struct_set(_, key, val)
    if s_proto.offsets[key] then
      buf:set(s_proto.offsets[key] + 1, val, s_proto.types[key])
    else
      error("field " .. key .. " does not exist")
    end
  end
  return setmetatable({}, { __index = struct_get, __newindex = struct_set,
                            __call = function () return buf end })
end

local function struct_byval(s_proto)
  local types = {}
  local size = s_proto.size
  for i = 0, size - 1, 4 do
    if size - i == 1 then
      types[#types + 1] = "char"
    elseif size - i == 2 then
      types[#types + 1] = "short"
    else
      types[#types + 1] = "int"
    end
  end
  return unpack(types)
end

local function st_size_iter(t, off, names, offsets, types, level)
  level = level or ""
  for fid, field in ipairs(t) do
    local oname, vtype = field[1], field[2]
    assert(type(oname)=="string", "bad field index "..fid)
    local name = level=="" and oname or level.."_"..oname
    if type(vtype)=="table" then
      if type(vtype.size)=="number" then
        local tt = {}
        for i=1,#(vtype.names) do
          assert(type(vtype.names[i])=="string", "bad field `"..name.."`")
          assert(vtype.types[vtype.names[i]], "bad field `"..name.."_"..vtype.names[i].."`")
          tt[#tt + 1] = {vtype.names[i], vtype.types[vtype.names[i]]}
        end
        off = st_size_iter(tt, off, names, offsets, types, name)
      else
        off = st_size_iter(vtype, off, names, offsets, types, name)
      end
    elseif type(vtype)=="number" then
      names[#names + 1] = name
      offsets[name] = off
      types[name] = vtype
      off = off + vtype
    else
      assert(type(vtype)=="string", "bad type with field "..name)
      names[#names + 1] = name
      off = math.ceil(off / alien.align(vtype)) * alien.align(vtype)
      offsets[name] = off
      types[name] = vtype
      off = off + alien.sizeof(vtype)
    end
  end
  return off
end

local st_type_meta = {
  __index = {
    new = struct_new,
    byval = struct_byval,
    pointer = "*",
    ptr = "*",
    p = "*",
  },
  __call = struct_new,
  __len = function(self)
    return self.size
  end,
  __bnot = function(self)
    return "*"
  end
}

function _M.defstruct(t)
  local off = 0
  local names, offsets, types = {}, {}, {}
  off = st_size_iter(t, off, names, offsets, types)
  local ret = {
    names = names,
    offsets = offsets,
    types = types,
    size = off,
  }
  setmetatable(ret, st_type_meta)
  return ret
end

function _M.byval(buf)
  if buf.size then
    local size = buf.size
    local types = { "char", "short"}
    local vals = {}
    for i = 1, size, 4 do
      if size - i == 0 then
        vals[#vals + 1] = buf:get(i, "char")
      elseif size - i == 1 then
        vals[#vals + 1] = buf:get(i, "short")
      else
        vals[#vals + 1] = buf:get(i, "int")
      end
    end
    return unpack(vals)
  else
    error("this type of buffer can't be passed by value")
  end
end

return alien
