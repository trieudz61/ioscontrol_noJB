$(document).ready(function () {
  ...
  var submit_cfg = function () {
    $.post("/set_user_conf", JSON.stringify({
      ...
      show_user_touch_pose: $("#show_user_touch_pose").is(":checked"),
      touchelf_httpd: $("#touchelf_httpd").is(":checked"),
      allow_cors: $("#allow_cors").is(":checked"),
      launch_daemons: $("#launch_daemons").is(":checked")
    }),
      function (cfg) {
        if (cfg.data != 0) {
          mdui.snackbar({
  ...
  $.post("/get_user_conf", "",
    function (cfg) {
      if (cfg.code == 0) {
        ...
        $("#show_user_touch_pose").attr("checked", cfg.data.show_user_touch_pose);
        $("#touchelf_httpd").attr("checked", cfg.data.touchelf_httpd);
        $("#allow_cors").attr("checked", cfg.data.allow_cors);
        $("#launch_daemons").attr("checked", cfg.data.launch_daemons);
        ...
        $("#show_user_touch_pose").on("click", submit_cfg);
        $("#touchelf_httpd").on("click", submit_cfg);
        $("#allow_cors").on("click", submit_cfg);
        $("#launch_daemons").on("click", submit_cfg)
      } else {
        mdui.snackbar({
          message: cfg.message
        })
